...
- (void)doRequestWithParams:(NSDictionary*)params
{
    ...
    YKLog(@"请求参数 = %@",paramsall);
    [self generateRequestWithUrl:self.requestUrl withParameters:paramsall];
    [_requestOperation start];
}

- (NSString *)encrytUrl:(NSString *)url
{
    NSMutableString *requesturl = [[NSMutableString alloc]initWithString:url];
    NSDictionary *params = [SAEncryt getEncrytParams];
    [params enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        [requesturl appendString:@"&"];
        [requesturl appendString:key];
        [requesturl appendString:@"="];
        [requesturl appendString:obj];
    }];
